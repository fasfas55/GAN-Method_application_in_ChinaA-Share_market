...
### Project Structure：
<pre> ```plaintext 📁 GAN-Method_application_in_ChinaA-Share_market │── README.md # Project documentation │── firm_char.py # Script to build firm characteristics dataset │── macro_data_preprocess.py # Script to collect and process macroeconomic predictors │── train_SDF.py # GAN model training for SDF │── train_beta.py # Beta prediction network training │── model_GAN.ipynb # Notebook for SDF generation & pricing results │ ├── config/ # Configuration files (JSON) ├── data/ # Input data (A-share monthly data, macro variables) ├── output/ # Model logs, trained weights, and evaluation results └── results/ # Final pricing metrics and plots ``` </pre>
