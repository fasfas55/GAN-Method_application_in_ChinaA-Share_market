...
def train_gan_sdf(
    ...
    # models
    # gen = SDFGenerator(F, M, hidden_dims=(128, 128, 64),
    #                    use_rnn=False, rnn_hidden=16, rnn_layers=1).to(device)
    gen = SDFGenerator(F, M, hidden_dims=(128, 128, 64),
                       use_rnn=False).to(device)
    moment = MomentNet(F, M, num_moments=30, hidden_dims=(128, 128),
                       use_rnn=False).to(device)

    # opt_gen = torch.optim.Adam(gen.parameters(), lr=lr_gen, weight_decay=1e-4) # add grad clipping
    ...
    for epoch in range(num_epochs):
        ...
        loss_g.backward()
        torch.nn.utils.clip_grad_norm_(gen.parameters(), max_norm=1.0) # fights exploding gradients
        opt_gen.step()
        ...
        if (epoch + 1) % 20 == 0:
            print("   [Debug] w_mean=", w_full.mean().item(), "w_std=", w_full.std().item())
